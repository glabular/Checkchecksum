...
{
    public static async Task<string> GetSha3_256ChecksumAsync(string path, IProgress<double> progress = null)
    {
        try
        {
            ...
            {
                sha3_256.TransformBlock(buffer, 0, read, null, 0);
                bytesRead += read;
            ...
            return ChecksumHelper.ByteArrayToString(sha3_256.Hash);
        }
        catch (IOException e)
        {
